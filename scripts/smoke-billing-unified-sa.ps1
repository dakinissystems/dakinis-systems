...
Write-Host "Supabase aplicado: 040-042, 044-045 | 043 pendiente smoke dual-write" -ForegroundColor DarkGray
...
if ($InternalKey) {
    ...
    if ($LiveSync -and $PlatformUserId -and $SaTenantId) {
        $syncBody = (@{
            ...
            stripeCustomerId     = "smoke_cus_test"
...
if ($LiveCheckout -and $SaJwt) {
    ...
    $checkout = Invoke-SmokeJson -Name "SA checkout probe" -Url "$StreamApiUrl/api/payments/checkout" `
        -Method "POST" -Headers @{ Authorization = "Bearer $SaJwt" } -Body $checkoutBody
...
Write-Host "  7. Migracion 043 solo tras 48h dual-write estable"
