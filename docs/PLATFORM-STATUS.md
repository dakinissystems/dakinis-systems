...
| **Hub** | 🟡 Desarrollo | Sí | Supabase `hub` | Christian | «Mi día» (no launcher) |
...
| **Knowledge** | 🟡 Scaffold | — | Supabase ⬜ | Christian | GitHub repo + ingest |
| **Landing** | 🟢 Activo | Sí | — | Christian | Funnel One-first |

**Prioridad plataforma (julio 2026):** Billing E2E Live · Supabase `022`/`023` · Hub «Mi día» · Knowledge + Event bus BullMQ.
...
└─────────┴─────────┴───────────────┴────────┴───────────┴──────────┘
    ↓
Internal API (/internal/) — proxy opcional · no desplegado Railway aún
```
...
| **Schema** | `hub` |
| **Estado** | 🔄 launcher hoy → **objetivo «Mi día»** |

Orden UX objetivo: Mi día → Timeline → Notificaciones → Apps → Widgets → Command Palette
...
| **Estado** | 🔄 API scaffold · **Railway deploy ⬜** |
| **Railway** | `knowledge-api` + `knowledge-worker` (dos servicios) |
...
| Hub | dakinis-hub | hub | hub.dakinissystems.com | — | ⬜ | ✅ |
...
| Knowledge | dakinis-knowledge | knowledge | knowledge.dakinissystems.com | 🔄 worker | ✅ | ✅ scaffold |
| Knowledge Worker | dakinis-knowledge | knowledge | interno | ✅ ingest | ✅ | — |
| Internal API | dakinis-internal-api | — | /internal/ | — | ⬜ | ⬜ no deploy |
...
3. **Hub «Mi día»** — centro del ecosistema
4. **Knowledge** — repo + schema + ingest mínimo
...
| 6 | Search + Knowledge | 🔄 scaffold (4082 + 4084) |
